...
/*
  friend std::ostream& operator<<(std::ostream& o, const Pole<T>& p)
  { for (int i=0; i<p.delka(); i++) o << p.pole[i] << " "; return o; }
*/
/*
  template <class X>
  friend std::ostream& operator<<(std::ostream&, const Pole<X>&);
*/
 };
...
Pole<T>::Pole(int pp) {
  ...
  //for (int i=0; i<pocetPrvku; i++) pole[i] = 0;
  for (int i=0; i<pocetPrvku; i++) pole[i] = T();
}
...
template <class T>
Pole<T>& Pole<T>::operator=(const Pole& p) {
  if (this!=&p) {
    delete [] pole;
    pocetPrvku = p.pocetPrvku;
    pole = new T[pocetPrvku];
